    ...
    return (
        <>
            ...
            >
                ...
                <button
                    onClick={e => { e.stopPropagation(); setModalPlaylist(true); }}
                    className="opacity-0 group-hover:opacity-100 transition-opacity w-7 h-7 rounded-full bg-surface border border-border flex items-center justify-center hover:bg-primary/10 hover:border-primary/30 cursor-pointer shrink-0"
                    title="Agregar a playlist"
                >
                    <span className="material-symbols-outlined text-text-muted text-base">add</span>
                </button>
            ...
            {modalPlaylist && (
                <ModalAgregarPlaylist
                    song={song}
                    onCerrar={() => setModalPlaylist(false)}
                />
            )}
        </>
    );
