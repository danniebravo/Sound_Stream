...
export default function Player() {
    const { currentSong, liked, toggleLike } = usePlayer();
    const cover = currentSong?.artworkUrl100?.replace("100x100", "60x60") ?? null;
        ...
        <footer className="fixed bottom-0 left-0 right-0 h-24 bg-surface border-t border-border flex items-center px-6 z-20">
            ...
            <div className="flex items-center gap-4 w-1/4 min-w-0">
                <div className="size-14 rounded-lg shadow-md bg-border overflow-hidden shrink-0 flex items-center justify-center">
                    {cover ? (
                        <img src={cover} alt={currentSong.trackName} className="w-full h-full object-cover" />
                    ) : (
                ...
                <div className="overflow-hidden min-w-0">
                    <p className="text-sm font-bold truncate text-text">
                        {currentSong?.trackName ?? "Sin reproducir"}
                    </p>
                    <p className="text-xs text-text-muted truncate">
                        {currentSong ? `${currentSong.artistName} · ${currentSong.collectionName ?? ""}` : "—"}
                    </p>
                </div>
                <button onClick={toggleLike} disabled={!currentSong} aria-label="Favorito"
                    className="text-text-muted hover:text-primary transition-colors ml-2 shrink-0 disabled:opacity-30">
                    <span className={`material-symbols-outlined text-lg ${liked ? "text-primary" : ""}`}
